...
print 'processing ',cyyyymmddhh

# ---- read in the associated analysis
    
iyyyymmddh_anal_end = int(dateshift(cyyyymmddhh,ileade))
iyyyymmddh_anal_begin = int(dateshift(cyyyymmddhh,ileadb))
...
idx_today_begin = int(np.where(yyyymmddhh_anal_list == iyyyymmddh_anal_begin)[0])
idx_today_end = int(np.where(yyyymmddhh_anal_list == iyyyymmddh_anal_end)[0])
...
if pmax_anal >= 0 and pmax_fcst >=0 :
    sumx_analysis_out = np.where(apcp_anal*conusmask_in > 0.0001, apcp_anal, zeros)
    sumlnx_analysis_out = np.where(apcp_anal*conusmask_in > 0.00001, \
        np.log(apcp_anal), zeros)
    npositive_analysis_out = np.where(apcp_anal*conusmask_in > 0, ones, zeros)
    nzeros_analysis_out = np.where(apcp_anal*conusmask_in == 0, ones, zeros)
    ...
    for imem in range(nmembers):
        ...
        sumx_forecast_out[imem_qmap,:,:] = sumx_forecast_out[imem_qmap,:,:] + \
            np.where(apcp_fcst*conusmask_in > 0, apcp_fcst, zeros)
        sumlnx_forecast_out[imem_qmap,:,:] = sumlnx_forecast_out[imem_qmap,:,:] + \
            np.where(apcp_fcst*conusmask_in > 0, np.log(apcp_fcst), zeros)
        npositive_forecast_out[imem_qmap,:,:] = npositive_forecast_out[imem_qmap,:,:] + \
            np.where(apcp_fcst*conusmask_in > 0, ones, zeros)
