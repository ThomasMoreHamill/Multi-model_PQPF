SUBROUTINE read_forecasts_on_CCPA (nxa, nya, nens, &
	infile_early, infile_late, ensemble_ccpa, ensemble_mean)

! ---- read netCDF file of forecast data.  Created 
!      earlier by precip_forecast_ccpa_2netcdf.py
...
REAL, ALLOCATABLE, DIMENSION(:,:,:) :: ensemble_ccpa_early, &
    ensemble_ccpa_late
...
ensemble_mean(:,:)  = 0.0
DO imem = 1,nens
    ensemble_mean(:,:) = ensemble_mean(:,:) + ensemble_ccpa(:,:,imem)
END DO
ensemble_mean(:,:) = ensemble_mean(:,:) / REAL(nens)

DEALLOCATE(ensemble_ccpa_early, ensemble_ccpa_late)

RETURN
END subroutine read_forecasts_on_CCPA
