PROGRAM supplemental_locations_eighth_degree

! purpose:  For every (i,j) in on the 1/8-degree Climatology-Calibrated
...
PARAMETER (nxa = 464)      ! CCPA 1/8-degree grid x dimensions for expanded grid.
PARAMETER (nya = 224)      ! CCPA 1/8-degree grid y dimensions for expanded grid
PARAMETER (nmonths = 12)   ! # months of the year
PARAMETER (nsupplemental = 50) ! max number of CCPA grid supplemental locations to be 
...
infile = TRIM(data_directory) // 'terrain_eighthdegree_from_NDFD.nc'
CALL read_terrain_heights(nxa, nya, infile, terrain, lonsa, latsa, &
    conusmask)
...
DO imonth = 1,12
    ...
    infile = TRIM(data_directory) // 'climatology_gamma_parameters_' // &
    ...
    PRINT *,'pre-processing to determine local standard devs of CDF  & terrain parameters'
    DO ixa = 1, nxa
    ...
    outfile = TRIM(data_directory) // 'supplemental_locations_CONUS_'//&
        cleadb //'_to_'//cleade//'_'//cmonths(imonth) // '.nc'
...
END PROGRAM supplemental_locations_eighth_degree
